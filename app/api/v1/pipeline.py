...
async def v1_pipeline_status(db: AsyncSession = Depends(get_async_db)):
    ...
    return {
        ...
        "schedule": [
            ...
            {"time": "08:00", "task": "Pipeline: Data collection", "type": "pipeline"},
            {"time": "08:30", "task": "Pipeline: Halal + Smart filter", "type": "pipeline"},
            {"time": "09:00", "task": "Pipeline: Full run (consensus→execute)", "type": "pipeline"},
            {"time": "10:00", "task": "Pipeline: Intraday full run", "type": "pipeline"},
            {"time": "11:00", "task": "Pipeline: Intraday full run", "type": "pipeline"},
            {"time": "12:00", "task": "Pipeline: Intraday full run", "type": "pipeline"},
            {"time": "13:00", "task": "Pipeline: Intraday full run", "type": "pipeline"},
            {"time": "14:00", "task": "Pipeline: Intraday full run", "type": "pipeline"},
            {"time": "15:00", "task": "Pipeline: Intraday full run", "type": "pipeline"},
            {"time": "15:30", "task": "Pipeline: Late-close check", "type": "pipeline"},
