...
# ── Financial Lexicon Boost for VADER ──
# VADER is general-purpose. These words have special meaning in finance.
_fin_lexicon = {
    'bull': 2.0, 'bullish': 2.5, 'bear': -2.0, 'bearish': -2.5,
    'upgrade': 2.5, 'downgrade': -2.5, 'outperform': 2.0, 'underperform': -2.0,
    'beat': 1.8, 'miss': -1.8, 'beats': 1.8, 'misses': -1.8,
    'surge': 2.5, 'surges': 2.5, 'soar': 2.5, 'soars': 2.5,
    'rally': 2.0, 'rallies': 2.0, 'rebound': 1.5, 'recover': 1.5,
    'plunge': -2.5, 'plunges': -2.5, 'crash': -3.0, 'crashes': -3.0,
    'tumble': -2.0, 'tumbles': -2.0, 'slump': -2.0, 'slumps': -2.0,
    'dividend': 1.5, 'buyback': 1.8, 'acquisition': 1.0,
    'bankruptcy': -3.5, 'lawsuit': -1.5, 'fraud': -3.0, 'recall': -2.0,
    'overweight': 2.0, 'underweight': -2.0, 'buy': 1.5, 'sell': -1.5,
    'strong buy': 3.0, 'strong sell': -3.0,
    'revenue growth': 2.0, 'earnings beat': 2.5, 'earnings miss': -2.5,
    'guidance raised': 2.5, 'guidance lowered': -2.5, 'guidance cut': -2.5,
    'breakout': 2.0, 'breakdown': -2.0, 'all-time high': 2.5,
    'record high': 2.5, 'record low': -2.5,
}
    ...
    try:
        ...
        try:
            ...
            if r:
                sig = r["swing_signal"]
                if "BUY" in sig:
                    votes_buy += 2 if "STRONG" in sig else 1
                    details.append({"Tool": "Halal Screener", "Signal": sig, "Vote": "BUY", "Score": r["swing_score"]})
                else:
                    votes_hold += 1
                    details.append({"Tool": "Halal Screener", "Signal": sig, "Vote": "HOLD", "Score": r["swing_score"]})
        except Exception as e:
        ...
        try:
            ...
            if prob >= 0.60:
                votes_buy += 1
                details.append({"Tool": "Monte Carlo", "Signal": f"Prob Profit {prob*100:.1f}%", "Vote": "BUY", "Score": round(prob*100,1)})
            elif prob <= 0.45:
                votes_sell += 1
                details.append({"Tool": "Monte Carlo", "Signal": f"Prob Profit {prob*100:.1f}%", "Vote": "SELL", "Score": round(prob*100,1)})
            else:
                votes_hold += 1
                details.append({"Tool": "Monte Carlo", "Signal": f"Prob Profit {prob*100:.1f}%", "Vote": "HOLD", "Score": round(prob*100,1)})
        except Exception as e:
        ...
        try:
            ...
            if chg > 1:
                votes_buy += 1
                details.append({"Tool": "LSTM", "Signal": f"+{chg:.1f}%", "Vote": "BUY", "Score": round(chg,2)})
            elif chg < -1:
                votes_sell += 1
                details.append({"Tool": "LSTM", "Signal": f"{chg:.1f}%", "Vote": "SELL", "Score": round(chg,2)})
            else:
                votes_hold += 1
                details.append({"Tool": "LSTM", "Signal": f"{chg:.1f}%", "Vote": "HOLD", "Score": round(chg,2)})
        except Exception as e:
        ...
        try:
            ...
            if chg > 1:
                votes_buy += 1
                details.append({"Tool": "Transformer", "Signal": f"+{chg:.1f}%", "Vote": "BUY", "Score": round(chg,2)})
            elif chg < -1:
                votes_sell += 1
                details.append({"Tool": "Transformer", "Signal": f"{chg:.1f}%", "Vote": "SELL", "Score": round(chg,2)})
            else:
                votes_hold += 1
                details.append({"Tool": "Transformer", "Signal": f"{chg:.1f}%", "Vote": "HOLD", "Score": round(chg,2)})
        except Exception as e:
        ...
        # --- 10. News Sentiment ---
        try:
            sr = run_sentiment_analysis(symbols=symbol, top_n=1)
            if sr and 'Sentiment' in sr[0]:
                ss = sr[0]['Sentiment']
                if ss >= 0.15:
                    votes_buy += 1
                    details.append({"Tool": "Sentiment", "Signal": f"Score {ss}", "Vote": "BUY", "Score": round(ss,3)})
                elif ss <= -0.15:
                    votes_sell += 1
                    details.append({"Tool": "Sentiment", "Signal": f"Score {ss}", "Vote": "SELL", "Score": round(ss,3)})
                else:
                    votes_hold += 1
                    details.append({"Tool": "Sentiment", "Signal": f"Score {ss}", "Vote": "HOLD", "Score": round(ss,3)})
        except Exception as e:
            details.append({"Tool": "Sentiment", "Signal": "ERROR", "Vote": "-", "Score": 0})

        total = votes_buy + votes_sell + votes_hold
        if votes_buy > votes_sell and votes_buy >= 3:
            verdict = "STRONG BUY"
            color   = "GREEN"
        elif votes_buy > votes_sell:
            verdict = "BUY"
            color   = "GREEN"
        elif votes_sell > votes_buy and votes_sell >= 3:
            verdict = "STRONG SELL"
            color   = "RED"
        ...
        confidence = round(max(votes_buy, votes_sell) / total * 100, 1) if total > 0 else 0
        ...
        summary = [{
            ...
            "TP1":          tp1,
            "TP2":          tp2,
            "Action":       "✅ ENTER" if "BUY" in verdict else "❌ AVOID" if "SELL" in verdict else "⏳ WAIT",
        }]
...
# ════════════════════════════════════════════
# TOOL 12: NEWS SENTIMENT ANALYSIS
# ════════════════════════════════════════════
def run_sentiment_analysis(symbols=None, top_n=20):
    """
    Scrapes news headlines from FinViz for halal stocks,
    runs VADER sentiment analysis with financial lexicon boost,
    returns scored results sorted by sentiment.
    """
    try:
        if symbols:
            stock_list = [s.strip().upper() for s in symbols.split(',')]
        else:
            stock_list = HALAL_STOCKS[:top_n]
        ...
        for symbol in stock_list:
            try:
                ...
                headline_details = []
                for h in headlines:
                    score = _vader.polarity_scores(h)
                ...
                results.append({
                    ...
                    "Top Headline": headlines[best_idx][:70],
                    "Top Score":    round(scores[best_idx], 3),
                })

                import time
                time.sleep(0.3)  # Respect FinViz rate limits

            except Exception as e:
                logger.error(f"Sentiment {symbol}: {e}")
                continue

        results.sort(key=lambda x: x["Sentiment"], reverse=True)
        return results if results else [{"Message": "No sentiment data available"}]
    except Exception as e:
        return [{"Error": str(e)}]


def get_sentiment_headlines(symbol="AAPL"):
    """
    Returns detailed headline-by-headline sentiment for one stock.
    """
    try:
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) '
        ...
        for row in news_table.find_all('tr')[:20]:
            ...
            date_cell = td[0].text.strip()
            if len(date_cell) > 8:
                current_date = date_cell.split(' ')[0]
                time_str = date_cell.split(' ')[1] if ' ' in date_cell else ""
            else:
                time_str = date_cell
        ...
        return results if results else [{"Message": "No headlines parsed"}]
    except Exception as e:
        return [{"Error": str(e)}]
...
# ════════════════════════════════════════════
def run_portfolio_optimizer(symbols="AAPL,MSFT,NVDA,AMZN,GOOG", capital=10000, method="max_sharpe"):
    """
    Optimizes portfolio weights using Modern Portfolio Theory.
    Methods: max_sharpe, min_risk, equal_risk, equal_weight
    """
    try:
        import numpy as np
        from scipy.optimize import minimize
        ...
        prices_dict = {}
        for t in tickers:
            df = fetch_yf(t, period='1y')
            if df is not None and len(df) > 60:
                prices_dict[t] = df['close'].values
            else:
                logger.error(f"Portfolio: skipping {t} - insufficient data")

        if len(prices_dict) < 2:
            return [{"Error": "Need at least 2 valid symbols with data"}]
        ...
        price_matrix = np.column_stack([prices_dict[t][-min_len:] for t in valid_tickers])
        ...
        # Current prices
        current_prices = {t: round(float(prices_dict[t][-1]), 2) for t in valid_tickers}
        ...
        bounds = tuple((0.02, 0.40) for _ in range(n))  # Min 2%, Max 40% per stock
        ...
        if method == "max_sharpe":
            ...
            opt_name = "Max Sharpe Ratio"

        elif method == "min_risk":
            def port_vol(w):
                return np.sqrt(np.dot(w.T, np.dot(cov_matrix, w)))
            result = minimize(port_vol, init_weights, method='SLSQP',
                            bounds=bounds, constraints=constraints)
            opt_weights = result.x
            opt_name = "Minimum Volatility"
            ...
            result = minimize(risk_contrib_diff, init_weights, method='SLSQP',
                            bounds=bounds, constraints=constraints)
            opt_weights = result.x
            opt_name = "Equal Risk Contribution"

        else:  # equal_weight
            opt_weights = init_weights
            opt_name = "Equal Weight"

        # Calculate portfolio stats
        port_ret, port_vol, port_sharpe = portfolio_stats(opt_weights)

        # Build results
        results = []
        for i, t in enumerate(valid_tickers):
            weight = round(float(opt_weights[i]) * 100, 1)
            alloc = round(capital * float(opt_weights[i]), 2)
            shares = int(alloc / current_prices[t]) if current_prices[t] > 0 else 0
            ann_ret = round(float(mean_returns[i]) * 100, 1)
            ann_vol = round(float(np.sqrt(cov_matrix[i][i])) * 100, 1)
            ind_sharpe = round(ann_ret / ann_vol, 2) if ann_vol > 0 else 0
        ...
        # Add summary row
        results.append({
            "Symbol":           f"PORTFOLIO ({opt_name})",
            "Weight %":         100.0,
            "Allocation $":     capital,
            "Shares":           "-",
            "Price":            "-",
            "Annual Return %":  round(float(port_ret) * 100, 1),
            "Annual Vol %":     round(float(port_vol) * 100, 1),
...
def run_correlation_matrix(symbols="AAPL,MSFT,NVDA,AMZN,GOOG"):
    """
    ...
    try:
        ...
        for i in range(len(valid_tickers)):
            for j in range(i+1, len(valid_tickers)):
                c = float(corr[i][j])
                if abs(c) > 0.7:
                    high_corr_pairs.append(f"{valid_tickers[i]}-{valid_tickers[j]}: {round(c,2)}")

        if high_corr_pairs:
            results.append({"Symbol": "HIGH CORRELATION WARNING", **{t: " | ".join(high_corr_pairs) for t in valid_tickers[:1]}} )
...
def run_pipeline(min_confidence=60, max_final=10, horizon=5, episodes=5):
    try:
        import datetime
        results_log = []

        # ============================================
        # المرحلة 1: فلتر سريع (EMA + RSI + Volume)
        # ============================================
        results_log.append({"Stage": "1 - Quick Filter", "Status": "Running...", "Stocks": len(RUSSELL_1000_HALAL)})
        ...
        for symbol in RUSSELL_1000_HALAL:
            try:
                ...
                # شروط الفلتر السريع
                trend_ok  = price > ema21_val
                ema_ok    = ema9_val > ema21_val
                rsi_ok    = 35 <= rsi_val <= 70
                vol_ok    = vol_ratio >= 1.0
                
                score = sum([trend_ok, ema_ok, rsi_ok, vol_ok])
                if score >= 3:
        ...
        top50 = quick_results[:50]
        results_log.append({"Stage": "1 - Quick Filter", "Status": "Done", "Stocks": len(top50)})

        # ============================================
        # المرحلة 2: Halal Screener كامل
        # ============================================
        results_log.append({"Stage": "2 - Halal Screener", "Status": "Running...", "Stocks": len(top50)})
        ...
        for item in top50:
            try:
                ...
                if r and r["swing_score"] >= 55:
                    screener_results.append(r)
            except:
                continue
        
        screener_results.sort(key=lambda x: x["swing_score"], reverse=True)
        top20 = screener_results[:20]
        results_log.append({"Stage": "2 - Halal Screener", "Status": "Done", "Stocks": len(top20)})

        # ============================================
        # المرحلة 3: USX Pro
        # ============================================
        results_log.append({"Stage": "3 - USX Pro", "Status": "Running...", "Stocks": len(top20)})
        
        top20_symbols = [r["symbol"] for r in top20]
        usx_all = run_usx_screener(min_score=6)
        usx_filtered = [x for x in usx_all if x.get("Symbol") in top20_symbols]
        usx_filtered.sort(key=lambda x: int(x.get("Score","0/10").split("/")[0]), reverse=True)
        top10 = [x["Symbol"] for x in usx_filtered[:10]]
        
        if not top10:
            top10 = top20_symbols[:10]
        
        results_log.append({"Stage": "3 - USX Pro", "Status": "Done", "Stocks": len(top10)})

        # ============================================
        # المرحلة 4: AI Consensus الكامل
        # ============================================
        results_log.append({"Stage": "4 - AI Consensus", "Status": "Running...", "Stocks": len(top10)})
        ...
        for symbol in top10:
            try:
                ...
                if consensus and len(consensus) > 0:
                    ...
                    row["ATR Pct"]      = screener_info.get("atr_pct", 0)
        ...
        results_log.append({"Stage": "4 - AI Consensus", "Status": "Done", "Stocks": len(final_results)})
        ...
        others      = [r for r in final_results if r.get("Verdict","") not in ["STRONG BUY","BUY","WEAK BUY"]]

        header = [{
            "Pipeline":      "Russell 1000 → Halal → Screener → USX → AI Consensus",
            "Stage 1":       f"Quick Filter: {len(RUSSELL_1000_HALAL)} → {len(top50)} stocks",
            "Stage 2":       f"Halal Screener: {len(top50)} → {len(top20)} stocks",
            "Stage 3":       f"USX Pro: {len(top20)} → {len(top10)} stocks",
            "Stage 4":       f"AI Consensus: {len(top10)} → {len(final_results)} stocks",
            "Strong BUY":    len(strong_buy),
            "BUY":           len(buy),
            "Weak BUY":      len(weak_buy),
            "Others":        len(others),
            "Best Opportunity": final_results[0]["Symbol"] if final_results else "None",
...
async def widgets():
    return {
        ...
        "pipeline_widget":{"name":"🚀 Full Pipeline","description":"Russell 1000 → Halal → Screener → USX → AI - Final Results","category":"Equity","type":"table","endpoint":"/pipeline","gridData":{"w":20,"h":9},"params":[{"paramName":"min_confidence","value":"60","label":"Min Confidence %","type":"text","show":True},{"paramName":"max_final","value":"10","label":"Max Final Stocks","type":"text","show":True},{"paramName":"horizon","value":"5","label":"Forecast Days","type":"text","show":True}]},
        "batch_consensus_widget":{"name":"Batch AI Consensus","description":"Scan all buy signals with all AI tools","category":"Equity","type":"table","endpoint":"/batch_consensus","gridData":{"w":20,"h":9},"params":[{"paramName":"min_swing_score","value":"55","label":"Min Swing Score","type":"text","show":True},{"paramName":"max_stocks","value":"10","label":"Max Stocks to Scan","type":"text","show":True},{"paramName":"horizon","value":"5","label":"Forecast Days","type":"text","show":True}]},
        ...
        "consensus_widget":{"name":"AI Consensus Vote","description":"All tools vote - final verdict","category":"Equity","type":"table","endpoint":"/consensus","gridData":{"w":20,"h":9},"params":[{"paramName":"symbol","value":"AAPL","label":"Symbol","type":"text","show":True},{"paramName":"horizon","value":"5","label":"Forecast Days","type":"text","show":True}]},
        "usx_pro_widget":{"name":"USX Pro V1.0 Screener","description":"10-point US swing trading system","category":"Equity","type":"table","endpoint":"/usx","gridData":{"w":20,"h":9},"params":[{"paramName":"min_score","value":"7","label":"Min Score (1-10)","type":"text","show":True}]},
        "sentiment_overview_widget":{"name":"News Sentiment Scanner","description":"Sentiment analysis of news for halal stocks using VADER + Financial Lexicon","category":"Equity","type":"table","endpoint":"/sentiment","gridData":{"w":20,"h":9},"params":[{"paramName":"symbols","value":"","label":"Symbols (comma separated, empty=top 20)","type":"text","show":True},{"paramName":"top_n","value":"20","label":"How many stocks","type":"text","show":True}]},
        "sentiment_detail_widget":{"name":"Sentiment Headlines","description":"Headline-by-headline sentiment detail for a single stock","category":"Equity","type":"table","endpoint":"/sentiment_detail","gridData":{"w":20,"h":9},"params":[{"paramName":"symbol","value":"AAPL","label":"Stock Symbol","type":"text","show":True}]},
        "portfolio_optimizer_widget":{"name":"Portfolio Optimizer","description":"Optimize portfolio weights using Max Sharpe, Min Risk, or Equal Risk methods","category":"Equity","type":"table","endpoint":"/portfolio","gridData":{"w":20,"h":9},"params":[{"paramName":"symbols","value":"AAPL,MSFT,NVDA,AMZN,GOOG","label":"Symbols (comma separated)","type":"text","show":True},{"paramName":"capital","value":"10000","label":"Capital ($)","type":"text","show":True},{"paramName":"method","value":"max_sharpe","label":"Method (max_sharpe/min_risk/equal_risk/equal_weight)","type":"text","show":True}]},
        "correlation_matrix_widget":{"name":"Correlation Matrix","description":"Shows correlation between stocks to avoid over-concentration","category":"Equity","type":"table","endpoint":"/correlation","gridData":{"w":20,"h":9},"params":[{"paramName":"symbols","value":"AAPL,MSFT,NVDA,AMZN,GOOG","label":"Symbols (comma separated)","type":"text","show":True}]},
...
@app.get("/pipeline_old")
async def pipeline_old(): return [{"msg":"use /pipeline"}]
async def pipeline(min_confidence:int=40, max_final:int=15, horizon:int=5, episodes:int=5):
    try: return run_pipeline(min_confidence=min_confidence, max_final=max_final, horizon=horizon, episodes=episodes)
    except Exception as e: return [{"Error":str(e)}]
@app.get("/pipeline_old")
async def pipeline_old(): return [{"msg":"use /pipeline"}]
async def pipeline(min_confidence:int=60, max_final:int=10, horizon:int=5, episodes:int=5):
    try: return run_pipeline(min_confidence=min_confidence, max_final=max_final, horizon=horizon, episodes=episodes)
    except Exception as e: return [{"Error":str(e)}]
...
@app.get("/sentiment")
async def sentiment(symbols:str=None, top_n:int=20):
    """News sentiment for multiple halal stocks"""
    try: return run_sentiment_analysis(symbols=symbols, top_n=top_n)
    except Exception as e: return [{"Error":str(e)}]

@app.get("/sentiment_detail")
async def sentiment_detail(symbol:str="AAPL"):
    """Headline-by-headline sentiment for one stock"""
    try: return get_sentiment_headlines(symbol=symbol)
    except Exception as e: return [{"Error":str(e)}]

@app.get("/portfolio")
async def portfolio(symbols:str="AAPL,MSFT,NVDA,AMZN,GOOG", capital:int=10000, method:str="max_sharpe"):
    """Portfolio optimization with multiple methods"""
    try: return run_portfolio_optimizer(symbols=symbols, capital=capital, method=method)
    except Exception as e: return [{"Error":str(e)}]

@app.get("/correlation")
async def correlation(symbols:str="AAPL,MSFT,NVDA,AMZN,GOOG"):
    """Correlation matrix between stocks"""
    try: return run_correlation_matrix(symbols=symbols)
    except Exception as e: return [{"Error":str(e)}]
